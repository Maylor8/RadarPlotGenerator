...
# Version:      1.0 (Using Rstudio version 1.3.959)
# Date:         13-09-2020
...
# When using Radar plot generator, please cite Rowlands et al. (2019) - DOI: 10.1186/s40798-019-0225-9.
# Please contact Ben Maylor (bm259@leicester.ac.uk) for any enquiries.

# 2. Library setup --------------------------------------------------------
  
  # Packages required to generate Radar plots
    library(ggplot2) # Verified using V3.3.1
...
# 3. Data setup -----------------------------------------------------------
  ...
  # Make data frame
    Data <- data.frame(Metric, Group, Mg)
...
# 4. Generate plot --------------------------------------------------------
  ...
  # Make plot
    Plot <- ggplot(data=Data, aes(x = Metric, y = Mg, group = Group)) +
  ...
  # Add Y axis labels. currently takes a bit of manual adjustment. One of the main aims for future releases is to make this less cumbersome. 
    geom_text(x = 0.75, y = (Yend/2), label = expression(paste("MX (m",italic("g"),")")), angle = 93, colour = "grey20", hjust = "left", size = 5) + # Y Axis title 
   ...
   #geom_text(x = 1, y = 500, label = "500", angle = 0, colour = "grey20", hjust = "right") +
   #geom_text(x = 1, y = 600, label = "600", angle = 0, colour = "grey20", hjust = "right") +
   #geom_text(x = 1, y = 700, label = "700", angle = 0, colour = "grey20", hjust = "right")
       

# 5. Export Radar plot ----------------------------------------------------

# this step is deactivated initially to save time until you are happy with the plot.
    
  # Export plot at high rendering quality to you current working directory
  # ggsave("PlotTest.jpg", width = 25, height = 20, units = "cm", dpi = 1500) # Input fill format here
